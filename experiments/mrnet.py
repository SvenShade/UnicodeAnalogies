...
# Purpose: Defines the MRNet model, "Scale-Localized Abstract Reasoning". CVPR 2021.
...
class MRNet(nn.Module):
    def __init__(self, model='mrnet'):
        ...
        self.model     = model
        self.MRNet     = nn.DataParallel(MRNet_model(), device_ids=[0,1])
        self.optimizer = torch.optim.Adam([param for param in self.parameters() if param.requires_grad],
                                           1e-3, betas=(0.9,0.999), weight_decay=1e-5)
        
    def compute_loss(self, output, model_output_heads, target):
        loss = F.cross_entropy(output, target)
        loss_head = [F.cross_entropy(output, target, reduction='none') for output in model_output_heads]
    ...
    def train_(self, image, target):
        ...
        output,mh = self(image)
        loss = self.compute_loss(output, mh, target)
        loss.backward()
        self.optimizer.step()
...
class MRNet_model(nn.Module):
    def __init__(self,
        ...
        super(MRNet_model, self).__init__()
        self.relu_before_reduce = relu_before_reduce
        self.levels = levels
        _dropout = {'high': 0.1, 'mid': 0.1, 'low': 0.1, 'mlp': 0.5} if dropout \
              else {'high': 0.,  'mid': 0.,  'low': 0.,  'mlp': 0. }
    ...
    def forward(self, x):
        ...
        return (out.view(-1, K0), out_multihead) if self.multihead else out.view(-1, K0)
